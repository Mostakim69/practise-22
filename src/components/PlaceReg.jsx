...
const PlaceReg = () => {
    ...
    const {setShowPlaceReg, axios, getToken, setIsOwner} = useAppContext()
    ...
    const onSubmitHandler = async (event)=> {
        try {
             ...
             const {data} = await axios.post(`/api/places/`, {name, contact, address, city}, {headers: {Authorization: `Bearer ${await getToken()}`}})

             if(data.success){
                toast.success(data.message)
                setIsOwner(true)
                setShowPlaceReg(false);
             } else{
    ...
    return (
        <div onClick={()=> setShowPlaceReg(false)} className='fixed top-0 bottom-0 left-0 right-0 z-100 flex items-center justify-center bg-black/70'>
            <form onSubmit={onSubmitHandler} onClick={(e)=> e.stopPropagation()} className='flex bg-white rounded-xl max-w-4xl max-md:mx-2'>
                ...
                <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
                    <img src={assets.closeIcon} alt="close-icon" className='absolute top-4 right-4 h-4 w-4 cursor-pointer' onClick={()=> setShowPlaceReg(false)} />
                    <p className='text-2xl font-semibold mt-6'>Register Your Place</p>
                    {/* Place name */}
